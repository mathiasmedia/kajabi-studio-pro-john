...
function buildSectionSettings(layout: SectionLayoutProps, flavor: SectionFlavor): Record<string, unknown> {
  ...
  if (flavor === "header") {
    ...
    if (layout.mobileHeaderBackgroundColor)
      settings.mobile_header_background_color = layout.mobileHeaderBackgroundColor;
...
function isSectionComponent(type: unknown): type is SectionComponent {
  return typeof type === "function" && "__kajabiSectionFlavor" in (type as object);
}
...
function isBlockComponent(type: unknown): type is BlockComponent {
  return typeof type === "function" && "kajabiType" in (type as object) && "serialize" in (type as object);
}
...
function walkSection(el: ReactElement): SerializedSection {
  ...
  const id = flavor === "header" ? "header" : flavor === "footer" ? "footer" : nextSectionId();
...
export function serializeTree(input: ReactNode | PageTrees): SerializeTreeResult {
  ...
  const trees: PageTrees = isPageTreesMap(input) ? (input as PageTrees) : { index: input as ReactNode };
